...
template <typename InputStream, typename T> struct Ok {
  ...
  Ok(InputStream &&stream, std::optional<T> val = {},
     std::optional<ParserError> err = {}, bool consumed = false)
...
public:
  ...
  using Result = std::variant<Ok<InputStream, T>, Error>;
  using OkCallback = function<Result(InputStream, std::optional<T>,
                                     std::optional<ParserError>)>;
  using ErrorCallback = function<Result(ParserError)>;

  // parser's continuations
  ...
  using UnParser = function<Result(Continuations &&)>;
  ...
  static bool isConsumed(const Result &r) {
    if (isOk(r)) {
      return std::get<Ok>(r).consumed;
    } else {
      return std::get<Error>(r).consumed;
    }
  }
  ...
  static auto empty() -> Parser<S, T> const {
    return Parser([](auto stream) { return Ok(std::move(stream), {}); });
  }
...
auto Parser<S, T>::map(Fn f)
  ...
  return {[=, unparser{std::move(unparser)}](Continuations &&cbs) ->

          typename PU::Result {
            return unparser(Continuations{
                .stream = std::move(cbs.stream),
                .cok =
                    [&](typename PU::InputStream stream, std::optional<U> val,
                        std::optional<ParserError> err) {
                      return cbs.cok(std::move(stream), f(val.value()), err);
                    },
                .cerr = cbs.cerr,
                .eok =
                    [&](typename PU::InputStream stream, std::optional<U> val,
                        std::optional<ParserError> err) {
                      return cbs.eok(std::move(stream), f(val.value()), err);
                    },
                .eerr = cbs.eerr});
          }};
}
...
auto Parser<S, T>::bind(Fn fma) -> typename function_traits<Fn>::return_type {
  ...
  return PTo{[=, unparser{std::move(unparser)}](
                 Continuations cbs) -> typename PTo::Result {
    // consumed case
